...
-- ============================================================
-- DEMO  —  delete everything below to use as a bare library
-- ============================================================
local W = UILib.CreatePanel({
	Name          = "UILibDemo",
	Title         = "UILib",
	SubTitle      = "restyled",
	Width         = 350,
	Height        = 360,
	Tabs          = { "Player", "World", "Data", "Setup" },
	Discord       = true,
	Search        = true,
	ClampToScreen = true,
...
local Player, World, Data, Setup = W.GetTab(1), W.GetTab(2), W.GetTab(3), W.GetTab(4)
...
-- ── Player ─────────────────────────────────────────────────
local move = UILib.CreateSection(Player, { Title = "Movement", Open = true }).Content
UILib.CreateSlider(move, {
	Label = "Walk speed", Min = 8, Max = 200, Default = 16, Step = 2,
	Format = "%d", Flag = "ws",
	OnChanged = function(v) local h = humanoid(); if h then h.WalkSpeed = v end end,
})
UILib.CreateSlider(move, {
	Label = "Jump power", Min = 20, Max = 250, Default = 50, Step = 5,
	Format = "%d", Flag = "jp",
...
UILib.CreateToggle(move, { Label = "Reapply on respawn", Default = true, Flag = "reapply" })
UILib.CreateKeybind(move, {
	Label = "Reset movement", Default = Enum.KeyCode.R, Flag = "resetkey",
	Tooltip = "Click the chip, then press a key",
	OnTrigger = function()
		local h = humanoid()
		if h then h.WalkSpeed = 16; h.UseJumpPower = true; h.JumpPower = 50 end
		UILib.ShowNotification("Reset", "Movement back to defaults.", 2)
	end,
})

local cam = UILib.CreateSection(Player, { Title = "Camera" }).Content
UILib.CreateSlider(cam, {
	Label = "Field of view", Min = 40, Max = 120, Default = 70, Format = "%d", Flag = "fov",
	OnChanged = function(v)
		local c = workspace.CurrentCamera
		if c then c.FieldOfView = v end
	end,
})
UILib.CreateGroup(cam, {
	Label = "Sensitivity", Options = { "Low", "Normal", "High" }, Default = 2, Flag = "sens",
})

-- ── World ──────────────────────────────────────────────────
local baseBright, baseClock = Lighting.Brightness, Lighting.ClockTime

UILib.CreateSlider(World, {
	Label = "Brightness", Min = 0, Max = 5, Default = math.floor(baseBright),
	Format = "%.0f", Flag = "bright",
	OnChanged = function(v) Lighting.Brightness = v end,
})
UILib.CreateGroup(World, {
	Label = "Time of day", Options = { "Reset", "Dawn", "Noon", "Dusk" },
	Default = 1, Flag = "tod",
	OnChanged = function(_, name)
		local c = { Dawn = 6.5, Noon = 12, Dusk = 18.2 }
		Lighting.ClockTime = (name == "Reset") and baseClock or c[name]
	end,
})
UILib.CreateToggle(World, {
	Label = "Global shadows", Default = Lighting.GlobalShadows, Flag = "shadows",
	OnChanged = function(on) Lighting.GlobalShadows = on end,
})
UILib.CreateColorPicker(World, { Label = "Highlight", Default = Theme.Accent, Flag = "hl" })
UILib.CreateDropdown(World, {
	Label = "Render preset", Options = { "Performance", "Balanced", "Fidelity" },
	Default = "Balanced", Flag = "render",
})
UILib.CreateDivider(World, { Text = "targets" })
UILib.CreateDropdown(World, {
	Label = "Track", Options = { "Players", "NPCs", "Drops", "Vehicles" },
	Default = { "Players" }, Multi = true, Placeholder = "none", Flag = "track",
})

-- ── Data ───────────────────────────────────────────────────
UILib.CreateParagraph(Data, {
	Title   = "About",
	Content = "Separation here comes from elevation and type weight rather than "
		.. "outlines, and the accent is spent only on state and live values.",
})

UILib.CreateDivider(Data, { Text = "live" })
local fpsKV  = UILib.CreateKeyValue(Data, { Label = "Frame rate", Value = "-" })
local pingKV = UILib.CreateKeyValue(Data, { Label = "Ping",       Value = "-" })
local upKV   = UILib.CreateKeyValue(Data, { Label = "Uptime",     Value = "0s" })
UILib.CreateKeyValue(Data, { Label = "GUI parent", Value = DefaultParent.Name })
UILib.CreateKeyValue(Data, {
	Label = "File API",
	Value = (type(writefile) == "function") and "yes" or "no",
})

UILib.CreateDivider(Data, { Text = "progress" })
local dl = UILib.CreateProgressBar(Data, { Label = "Downloading", Default = 0 })
local hp = UILib.CreateProgressBar(Data, {
	Label = "Health", Default = 100, Format = "%d HP",
})
UILib.CreateButton(Data, {
	Text = "Run to 100%", Style = "Primary",
	OnClick = function()
		task.spawn(function()
			for i = 0, 100, 4 do
				if not (W.Gui and W.Gui.Parent) then return end
				dl.Update(i)
				task.wait(0.04)
			end
			UILib.ShowNotification("Done", "Transfer finished.", 2)
		end)
	end,
})

UILib.CreateDivider(Data, { Text = "log" })
local log = UILib.CreateStatusLog(Data, { Height = 110, MaxLines = 60 })

UILib.CreateCode(Data, {
	Language = "lua", Height = 56,
	Text = 'local UILib = loadstring(game:HttpGet(url))()\nUILib.CreatePanel({ Title = "Hi" })',
})

-- ── Setup ──────────────────────────────────────────────────
UILib.CreateDropdown(Setup, {
	Label = "Theme", Options = UILib.ThemeNames, Default = "Ice", Flag = "theme",
	Tooltip = "Retints live, nothing rebuilds",
	OnChanged = function(name)
		UILib.SetTheme(name)
		log.Log("theme -> " .. name, Theme.Accent)
	end,
})

local sizes = UILib.CreateHStack(Setup, { Height = Theme.RowHeight }).Frame
UILib.CreateButton(sizes, { Text = "Compact", OnClick = function() W.SetSize(310, 300) end })
UILib.CreateButton(sizes, { Text = "Roomy",   OnClick = function() W.SetSize(460, 520) end })

UILib.CreateTextInput(Setup, {
	Label = "Panel title", Placeholder = "UILib", Width = 100, MaxLength = 18,
	OnSubmit = function(t) if t ~= "" then W.SetTitle(t) end end,
})

UILib.CreateDivider(Setup, { Text = "profiles" })

local pname = UILib.CreateTextInput(Setup, {
	Label = "Name", Placeholder = "default", Default = "default", Width = 100,
})
local pdrop = UILib.CreateDropdown(Setup, {
	Label = "Saved",
	Options = (function()
		local l = UILib.ListConfigs()
...
local function refreshProfiles()
	local l = UILib.ListConfigs()
	pdrop.SetItems((#l > 0) and l or { "(none)" }, true)
end

local prow = UILib.CreateHStack(Setup, { Height = Theme.RowHeight }).Frame
UILib.CreateButton(prow, {
	Text = "Save", Style = "Primary",
	OnClick = function()
		local n = pname.GetValue()
		if n == "" then n = "default" end
		local ok, err = UILib.SaveConfig(n)
		refreshProfiles()
		log.Log(ok and ("saved " .. n) or ("save failed: " .. tostring(err)),
			ok and Theme.Accent or Color3.fromRGB(226, 92, 88))
	end,
})
UILib.CreateButton(prow, {
	Text = "Load",
	OnClick = function()
		local n = pdrop.GetValue()
		if not n or n == "(none)" then return end
		local ok, err = UILib.LoadConfig(n)
		log.Log(ok and ("loaded " .. n) or ("load failed: " .. tostring(err)),
			ok and Theme.Accent or Color3.fromRGB(226, 92, 88))
	end,
})

local arow = UILib.CreateHStack(Setup, { Height = Theme.RowHeight }).Frame
UILib.CreateButton(arow, {
	Text = "Autoload",
	OnClick = function()
		local n = pdrop.GetValue()
		if not n or n == "(none)" then return end
...
UILib.CreateButton(arow, {
	Text = "Delete", Style = "Danger", Confirm = true, ConfirmText = "Delete?",
	OnClick = function()
		local n = pdrop.GetValue()
		if not n or n == "(none)" then return end
		UILib.DeleteConfig(n)
		refreshProfiles()
		log.Log("deleted " .. n)
	end,
})

UILib.CreateKeyValue(Setup, { Label = "Autoload", Value = UILib.GetAutoload() or "none" })
UILib.CreateSpace(Setup, { Height = 4 })
UILib.CreateButton(Setup, {
	Text = "Unload everything", Style = "Danger", Confirm = true, ConfirmText = "Sure?",
	OnClick = function() UILib.Unload() end,
})
...
task.spawn(function()
	...
	local frames, clock, started = 0, 0, os.clock()
	...
	while W.Gui and W.Gui.Parent do
		task.wait(1)
		if clock > 0 then
			fpsKV.SetValue(math.floor(frames / clock + 0.5) .. " fps")
			frames, clock = 0, 0
		end
		local ok, v = pcall(function()
			return Stats.Network.ServerStatsItem["Data Ping"]:GetValue()
		end)
		pingKV.SetValue(ok and (math.floor(v + 0.5) .. " ms") or "n/a")
		upKV.SetValue(math.floor(os.clock() - started) .. "s")
		local h = humanoid()
		if h then hp.Update(h.Health / math.max(h.MaxHealth, 1) * 100) end
	end
	conn:Disconnect()
end)

UILib.ShowNotification("UILib", "Filter with the search box. RightControl hides.", 4)
